...
function setImage(src, value) {
  ...
  if (!isValidValue(value)) {
    alert("Valor inválido para a miniatura!");
    return;
  }
...
document
  .getElementById("finalizarPagamentoButton")
    ...
    fetch("http://localhost:3000/create-preference", {
      ...
      .then((data) => {
        if (data.preferenceId) {
          window.location.href = `https://www.mercadopago.com.br/checkout/v1/payment/redirect?preference-id=${data.preferenceId}`;
        } else {
          console.error("PreferenceId não foi gerado corretamente.");
        }
      })
      .catch((error) => {
        console.error("Erro ao gerar a preferência:", error);
        alert("Erro ao tentar processar o pagamento. Tente novamente.");
      });
  });
